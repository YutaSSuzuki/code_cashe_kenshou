# EC2 and CloudWatch Verification

## Status

Complete: EC2 restore, Code Cache full reproduction, CloudWatch collection, 64MB comparison, and flushing behavior verified

## Goal

GitHubへ登録した資材からUbuntu EC2上に検証環境を復元し、次を一連の証跡として確認した。

1. 32MB、flushing無効でCode Cache満杯を再現する。
...
6. 32MB、flushing有効でSweeper回収によりJITが継続することを確認する。
...
## Phase 3: Reproduce Full Code Cache
...
| Option | Purpose |
| --- | --- |
| `-XX:ReservedCodeCacheSize=10m` | 満杯を短時間で強制する。ローカルの機構確認用であり、EC2本検証では32MBを使用した |
...
4. `full_count=0`なら1,000 classずつ追加する。
...
5. 32MBで満杯になったときと同じclass数・iterationsをGeneratorへ指定する。
...
## Phase 5: Verify Code Cache Flushing with 32 MB

1. 64MB JVMを停止する。
2. `-XX:-UseCodeCacheFlushing`を外し、`-XX:ReservedCodeCacheSize=32m`で再起動する。
3. Generatorを同じbatch条件で20回実行する。
4. batchごとに`jcmd "$PID" Compiler.codecache`を保存する。
5. `used`と`nmethods`が途中で減少し、`Compilation: enabled`が維持されることを確認する。

回収の判定は`used`だけではなく、`nmethods`も同時に見る。両方が減っていれば、コンパイル済みmethodが
SweeperによりCode Cacheから回収されたと判断できる。
...
| 32MB full | `full_count > 0`かつ`stopped_count > 0` |
| CloudWatch | pool used/max、class数、warning logを確認できる |
| Delay | 32MB満杯時のColdがHotより明確に遅い |
| 64MB capacity | 同じGenerator条件で`full_count=0`、Compilation enabled |
| Delay recovery | 64MB条件のColdが32MB満杯時より短い |
| Flushing | 32MB、flushing有効で`used`と`nmethods`が減少し、Compilation enabledを維持 |
